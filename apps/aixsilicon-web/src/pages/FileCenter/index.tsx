...
export default function FileCenter() {
    ...
    const { data: fileSpaces = [] } = useFileSpaces();
    const { data: toolPages } = useToolsInfinite({});
    const tools = useMemo(() => toolPages?.pages.flat() ?? [], [toolPages]);
    const registeredTools = useMemo(() => tools.filter((tool) => tool.file_space_enabled), [tools]);
    ...
    const [upToolId, setUpToolId] = useState('');
    ...
    const namespaces = useMemo(() => Array.from(new Set([...collectNamespaces(tree), ...fileSpaces.map((space) => space.namespace)])).sort(), [tree, fileSpaces]);
    const writableSpaces = useMemo(() => fileSpaces.filter((space) => (LEVEL_RANK[space.access_level] ?? 0) >= LEVEL_RANK.write), [fileSpaces]);
    const selectedFiles = useMemo(() => files.filter((file) => selectedIds.has(file.id)), [files, selectedIds]);

    const resetUpload = () => {
        setUpToolId(''); setUpGroup(''); setUpFunc(''); setUpNs(''); setUpTags(''); setUpFiles([]); setUploadProgress({});
    };
    ...
    const handleUpload = async () => {
        if (!upToolId || !upGroup || !upFunc || !upNs.trim()) return message.warning('请选择上传目标工具');
        ...
        try {
            for (const file of upFiles) {
                ...
                const uploadData = {
                    file, tool_id: upToolId, group_name: upGroup, func_type: upFunc, namespace: upNs.trim(),
