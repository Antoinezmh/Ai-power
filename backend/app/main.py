...
async def readiness():
    ...
    ready = checks["database"] == "ok" and (not settings.REQUIRE_REDIS or checks["redis"] == "ok")
    if not ready:
...
async def startup():
    ...
    async with AsyncSessionLocal() as db:
        ...
        if settings.APP_ENV.lower() == "production" and not user_count and not settings.SSO_ENABLED and not (
            settings.BOOTSTRAP_ADMIN_USERNAME
            and settings.BOOTSTRAP_ADMIN_EMAIL
            and settings.BOOTSTRAP_ADMIN_PASSWORD
        ):
            raise RuntimeError("Empty database requires bootstrap administrator credentials or SSO")
