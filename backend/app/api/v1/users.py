...
async def list_users(
    ...
    roles_by_user = await UserService.get_users_roles(db, [user.id for user in users])
    result = []
    for u in users:
        result.append(UserResponse(
            ...
            roles=roles_by_user[u.id],status=u.status
        ))
    return result
...
async def update_user(
    ...
    if current_user.id == user_id and update_data.get("status") == "inactive":
