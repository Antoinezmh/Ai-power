# ai-platform 第三方工具集成标准规范（历史方案）

> **状态：已归档，不可直接用于当前部署。** 本文保留早期集成思路，其中的
> `/cp/`、`/rag/`、宿主目录及前端 token 约定与 1.1 实现不完全一致。
> 当前工具注册和运行规范以 [`third_party/README.md`](third_party/README.md)
> 为准，服务器操作以 [`DEPLOYMENT.md`](DEPLOYMENT.md) 为准。当前静态工具使用
> `/tools/<namespace>/`，站内动态工具使用 `/<namespace>/`，且必须通过独立工具域名的启动票据和会话校验。
